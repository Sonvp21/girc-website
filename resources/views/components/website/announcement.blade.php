<div @class(['mt-5' => !request()->routeIs('home')])>
    <x-website.partials.header
        title="<a href='{{ route('announcements.index') }}' class='text-red-500'>{{ __('web.announcement_home') }}</a>"
        ...
        @foreach ($announcements as $announcement)
            <li>
                <a href="{{ route('announcements.show', $announcement) }}"
                    class="inline-block gap-2 py-4 text-xs hover:text-red-600">
                    <div
                        ...
                        <div class="flex h-auto flex-col border-b-2 border-red-500 bg-white text-center ">
                            <span
                                class="relative top-0.5 text-base font-bold leading-3">{{ $announcement->published_at->translatedFormat('d') }}</span>
                            <span
                                class="h-4 text-[0.5rem] font-bold capitalize text-[#fd9f1b]">{{ $announcement->published_at->translatedFormat('l') }}</span>
                        </div>
                    </div>
