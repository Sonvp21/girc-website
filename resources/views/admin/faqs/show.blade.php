                ...
                <div class="border-gray-200 border-b bg-white p-6">
                    <div class="stats shadow">
                        ...
                        <div class="stat">
                            ...
                            @lang('admin.faqs.read_at'):{{ $faq->read_at ? $faq->read_at : 'Not read yet' }}</p>
                        </div>

                    </div>
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-1 w-fit">
                        <div class="chat chat-start flex">
                            <h3 class="text-gray-900 text-lg font-medium leading-6">@lang('admin.faqs.question'):</h3>
                            <div class="chat-bubble">{!! $faq->question !!}</div>
                            <p class="text-gray-500 mt-1 max-w-2xl text-sm ml-auto">
                                @lang('admin.faqs.created_at'):{{ $faq->createdAtVi }}</p>
                        </div>
                        ...
                        <form action="{{ route('admin.faqs.update', $faq) }}" method="POST">
                            ...
                            @csrf
                            <div class="flex">
                                <h3 class="text-gray-900 text-lg font-medium leading-6">@lang('admin.faqs.answer'):</h3>
                                <p class="text-gray-500 mt-1 max-w-2xl text-sm ml-auto">
                                    @lang('admin.faqs.answer_at'):{{ $faq->answer_at ? $faq->answer_at : 'Not answer yet' }}</p>
                            </div>

                            <input type="hidden" name="id" value="{{ $faq->id }}" />
                            <div class="mt-1 flex items-center text-sm">
                                <svg style="width: 20px;position: absolute;"
                                    class="mr-2 h-4 w-4 flex-none text-teal-500" xmlns="http://www.w3.org/2000/svg"
                                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <textarea style="margin-left: 30px;" name="answer" class="w-full rounded-md border-gray-300 shadow-sm h-auto" required>{!! $faq->answer ?: old('answer') !!}</textarea>
                            </div>
                            <div class="mt-3" style="margin-left: 24%;">
