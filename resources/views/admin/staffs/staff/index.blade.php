                        ...
                        <tbody>
                            @foreach ($staffs as $staff)
                                <tr>
                                    ...
                                    <td>{{ $staff->name }}</td>
                                    <td>{{ $staff->departments->pluck('name')->join(', ') }}</td>
                                    <td>{{ $staff->createdAtVi }}</td>
                                    <td>{{ $staff->updatedAtVi }}</td>

                                    <td class="flex gap-3">
                                        <a href="{{ route('admin.staffs.edit', $staff->id) }}"><x-heroicon-s-pencil-square
                                                class="size-4 text-green-600" /></a>
                                        <form id="delete-form-{{ $staff->id }}"
                                            action="{{ route('admin.staffs.destroy', ['staff' => $staff->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
