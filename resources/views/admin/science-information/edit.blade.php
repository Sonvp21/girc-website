        ...
        <div class="mt-6">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="bg-white px-8 pb-8 pt-0 shadow sm:rounded-lg">
                    <form action="{{ route('admin.science-information.update', $scienceInformation) }}" method="POST"
                        class="needs-validation space-y-4" novalidate enctype="multipart/form-data">
                        @csrf
                        @method('patch')
                        ...
                        <div class="flex gap-4">
                            <label class="form-control">
                                ...
                                <x-admin.forms.calendar name="published_at"
                                    value="{{ $scienceInformation->published_at }}" />
                            </label>
                        </div>
                        <div class="flex gap-4">
                            <label class="form-control w-full">
                                ...
                                <input type="text" name="title" placeholder="Type here"
                                    value="{{ $scienceInformation->title }}" @class([
                                        'input',
                                        'input-bordered',
                            ...
                            <label class="form-control w-full">
                                ...
                                <input type="text" name="title_en" value="{{ $scienceInformation->title_en }}"
                                    placeholder="Title english(if have)" @class([
                                        'input',
                                        'input-bordered',
                        ...
                        <label class="form-control w-full">
                            ...
                            <select name="keep_on_top" @class(['input', 'input-bordered', 'w-full'])>
                                <option value="0" {{ $scienceInformation->keep_on_top ? '' : 'selected' }}>
                                    @lang('admin.false')
                                </option>
                                <option value="1" {{ $scienceInformation->keep_on_top ? 'selected' : '' }}>
                                    @lang('admin.true')
                                </option>
                            </select>
                        </label>
                        <div class="flex justify-end gap-4">
                            <a href="{{ route('admin.science-information.index') }}"
